...
class oneNoteData: NSObject, NSURLConnectionDelegate, NSURLConnectionDataDelegate
{
    ...
    func connectToOneDrive()
    {
        ...
        {
            ODClient.setMicrosoftAccountAppId(CLIENT_ID, scopes: OneNoteScopeText)
...
//        let request = NSMutableURLRequest(url: url)
        ...
        if liveClient != nil
        {
            liveClient.drive().items().requestURL.    request().getWithCompletion({ (ODdrive, error) -> Void in
