...
extension CloudKitInteraction
{
    ...
    {
        let predicate: NSPredicate = NSPredicate(format: "(updateTime >= %@) AND (teamID == \(myTeamID))", myDatabaseConnection.getSyncDateForTable(tableName: "Outline") as CVarArg)
    ...
    {
        let sem = DispatchSemaphore(value: 0)
        let predicate = NSPredicate(format: "(outlineID == \(sourceRecord.outlineID)) AND (teamID == \(myTeamID))") // better be accurate to get only the record you need
